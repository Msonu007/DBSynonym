...
if st.sidebar.button("Update Config"):
    resp = requests.post(
        ...
        json={"cache_backend": cache_backend, "ttl": ttl, "redis_url": redis_url}
    )
    status_ = st.sidebar.success(resp.json().get("status"))
    info_box = st.sidebar.info("Server restarting... Please wait 5 seconds.")
    time.sleep(10)
    status_.empty()
    info_box.empty()
    try:
        resp = requests.get(API_URL, params={"ttl": ttl, "cache_backend": cache_backend, "redis_url": redis_url})
        data = resp.json()
    ...
    params = {"ttl": ttl, "cache_backend": cache_backend, "redis_url": redis_url}
    try:
